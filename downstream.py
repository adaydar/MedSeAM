...
def save_models(bigru_model, cnn_models, fc_layer,cross_attention):
    ...
    torch.save(cnn_models.state_dict(), CNN_MODEL_PATH_DOWNSTREAM)

    torch.save(bigru_model.state_dict(), BIGRU_MODEL_PATH_DOWNSTREAM )
    
    torch.save(fc_layer.state_dict(), FC_lAYER_PATH_DOWNSTREAM)
    
    torch.save(cross_attention.state_dict(), CROSS_ATTENTION_PATH_DOWNSTREAM)

dataset = ImageDataset(root_dir1,root_dir2)
